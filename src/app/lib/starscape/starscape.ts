...
export default class StarScape {
    ...
    validateStars(oldWidth: number, oldHeight: number) {
        ...
        if (oldWidth > width || oldHeight > height) {
            for (let x = this.starArray.length - 1; x > 0; x -= 1) {
                if (
                    this.starArray[x].x > width ||
                    this.starArray[x].y > height
                ) {
        ...
        if (oldWidth < width || oldHeight < height) {
            ...
            let maxWidth =
                oldWidth >= width ? width : Math.max(oldWidth, width);

            let minHeight =
                oldHeight >= height ? 0 : Math.min(oldHeight, height);
            let maxHeight =
    ...
    validateShootingStars() {
        for (let x = 0; x < this.shootingStarArray.length; x += 1) {
            if (this.shootingStarArray[x].isDead)
                this.shootingStarArray.splice(x, 1);
        }
    }
