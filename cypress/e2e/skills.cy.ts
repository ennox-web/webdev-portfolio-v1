describe('verify skills elements', () => {
    ...
    it('loads skills section elements', () => {
        cy.get('[data-cy="skills-title-lang"]').should('be.visible').and('have.text', "Languages");
        cy.get('[data-cy="skills-lang-chip"]').should('be.length', 6);

        cy.get('[data-cy="skills-title-tech"]').should('be.visible').and('have.text', "Technical");
        cy.get('[data-cy="skills-tech-chip"]').should('be.length', 12);

        cy.get('[data-cy="skills-title-devop"]').scrollIntoView();
        cy.get('[data-cy="skills-title-devop"]').should('be.visible').and('have.text', "DevOps");
